...
// Function to set thrust direction manually
void set_thrust_direction(Satellite *sat, double x, double y, double z) {
    ...
    if (magnitude > 0) {
        ...
        sat->thrust_direction[2] = z / magnitude;
    } else {
        sat->thrust_direction[0] = 0.0;
        sat->thrust_direction[1] = 0.0;
        sat->thrust_direction[2] = 0.0;
    }
}
...
void update_position(Satellite *sat, Planet *planet, double timestep) {
    ...
    if (sat->fuel_mass > 0 && sat->thrust > 0) {
        ...
        az_thrust = thrust_magnitude * sat->thrust_direction[2];
