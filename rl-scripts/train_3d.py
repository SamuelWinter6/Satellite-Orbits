...
SERVER_PORT = 65433           # Port for 3D simulation
...
def main():
    ...
    try:
        ...
        print(f"Connected to 3D simulation server at {SERVER_ADDRESS}:{SERVER_PORT}")
        ...
        for step in range(10):  # Simulate 10 steps
            ...
            thrust_direction = [random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(-1, 1)]  # Random 3D unit vector
            direction_magnitude = (thrust_direction[0] ** 2 + thrust_direction[1] ** 2 + thrust_direction[2] ** 2) ** 0.5
